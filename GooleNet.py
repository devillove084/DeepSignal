...
class GoogleNet(nn.Module):
    def __init__(self,
        ...
        if blocks is None:
            blocks = [Inception, BasicConv2d]
        if init_weights is None :
            init_weights = True
            assert len(blocks) == 2

        conv_block = blocks[1]
        inception_block = blocks[0]

        self.transform_input = transform_input
        self.conv1 = conv_block(3, 64, kernel_size=7, stride=1, padding=1)
